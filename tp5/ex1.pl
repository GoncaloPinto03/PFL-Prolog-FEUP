...
% a)
children(P, C) :- findall(Child, parent(P, Child), C).

% b)
children_of([], []).
children_of([Person | People], [Person-Children | Pairs]) :- children(Person, Children), children_of(People, Pairs).

% c)
family(F) :- setof(X, Y^(parent(X, Y); parent(Y, X)), F).

% d)
couple(M-W) :- parent(M, X), parent(W, X), M \= W.

% e)
couples(L) :- setof(X, couple(X), L).

% f)
spouse_children(P, S/C) :- couple(P-S), children(P, C).
